...
func Init() {
	...
	if err != nil {
		fmt.Errorf("could not get initial namespace: %v", err)
	}
	initNl, err = netlink.NewHandle(getSupportedNlFamilies()...)
	if err != nil {
		fmt.Errorf("could not create netlink handle on initial namespace: %v", err)
	}
	err = initNl.SetSocketTimeout(NetlinkSocketsTimeout)
	if err != nil {
		fmt.Errorf("Failed to set the timeout on the default netlink handle sockets: %v", err)
	}
}
...
func getSupportedNlFamilies() []int {
	...
	if err := loadXfrmModules(); err != nil {
		if checkXfrmSocket() != nil {
			fmt.Errorf("Could not load necessary modules for IPSEC rules: %v", err)
		} else {
	...
	if err := loadNfConntrackModules(); err != nil {
		if checkNfSocket() != nil {
			fmt.Errorf("Could not load necessary modules for Conntrack: %v", err)
		} else {
